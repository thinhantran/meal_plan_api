...
class AuthTest {
  ...
  @Test
  void testChangePasswordFailure401() {
    Response response = given().when()
        .formParam("username", "java")
        .formParam("password", "admin")
        .post("/auth/login/user");
    String refreshToken = response.getBody().jsonPath().getString("accessToken");
    given().when()
        .header("Authorization", "Bearer " + refreshToken)
        .formParam("oldPassword", "admi")
        .formParam("newPassword", "administrator")
        .put("/auth/users")
        .then()
        .statusCode(401);
  }
  ...
  void testChangePasswordSuccessful() {
    ...
    given().when()
        .formParam("username", "c++")
        .formParam("password", "administrator")
        .post("/auth/login/user")
        .then()
        .statusCode(200);
  }
